...
export default function CreatePostLaptopForm(props: Props) {
  ...
  const [microprocessor, setMicroprocessor] = useState<number | string>(
    props.data?.Microprocessor?.id || ''
  );
  const [ram, setRam] = useState<number | string>(props.data?.Ram?.id || '');
  const [monitorCard, setMonitorCard] = useState<number | string>(
    props.data?.Monitor_card?.id || ''
  );
