...
export default function CreatePostServiceForm(props: Props) {
  ...
  const [url, setUrl] = useState(getParentUrl.Service);
  ...
  useEffect(() => {
    fetchServiceGuaranteeList().then((res) =>
      setGuaranteeList(res.data.data || [])
    );
    fetchServiceSellerInformationList().then((res) =>
      setSellerInformationList(res.data.data || [])
    );
    fetchServiceUsageStatusList().then((res) =>
