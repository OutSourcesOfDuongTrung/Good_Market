...
export default function SellerInfoVehiclePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/vehicle/seller-information/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchSellerInfoVehicleList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/vehicle/seller-information/${id}/`)
      .then((res) => {
        message.success('Xóa thành công');
        mutate('fetchSellerInfoVehiclePageList');
      })
      .catch((err) => {
        message.error('Thao tác thất bại');
      });
  ...
  const fetchSellerInfoVehiclePageList = useCallback(async () => {
    await instanceAxios
      .get(`/vehicle/seller-information/`, {
        params: {
  ...
  useSWR('fetchSellerInfoVehiclePageList', fetchSellerInfoVehiclePageList);

  useEffect(() => {
    fetchSellerInfoVehiclePageList();
  }, [fetchSellerInfoVehiclePageList]);
  // useEffect(() => {
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/vehicle/seller-information/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchSellerInfoVehiclePageList');
