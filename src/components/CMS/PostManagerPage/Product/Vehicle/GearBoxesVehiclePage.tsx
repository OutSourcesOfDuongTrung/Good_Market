...
export default function GearBoxesVehiclePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/vehicle/gearboxes/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchGearBoxesVehiclePageList');
      })
      .catch((err) => {
  ...
  const fetchGearBoxesVehiclePageList = useCallback(async () => {
    await instanceAxios
      .get(`/vehicle/gearboxes/`, {
        params: {
  ...
  useSWR('fetchGearBoxesVehiclePageList', fetchGearBoxesVehiclePageList);

  useEffect(() => {
    fetchGearBoxesVehiclePageList();
  }, [fetchGearBoxesVehiclePageList]);
  // useEffect(() => {
