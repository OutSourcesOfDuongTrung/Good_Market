...
export default function FactoryYearVehiclePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/vehicle/years-of-manufacture/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchFactoryYearVehicleList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/vehicle/years-of-manufacture/${id}/`)
      .then((res) => {
  ...
  const fetchFactoryYearVehicleList = useCallback(async () => {
    await instanceAxios
      .get(`/vehicle/years-of-manufacture/`, {
        params: {
  ...
  useSWR('fetchFactoryYearVehicleList', fetchFactoryYearVehicleList);

  useEffect(() => {
    fetchFactoryYearVehicleList();
  }, [fetchFactoryYearVehicleList]);
  // useEffect(() => {
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/vehicle/years-of-manufacture/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchFactoryYearVehicleList');
