...
export default function CompanyVehiclePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/vehicle/companies/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchCompanyVehicleList');
      })
      .catch((err) => {
  ...
  const fetchCompanyVehicleList = useCallback(async () => {
    await instanceAxios
  ...
  useSWR('fetchCompanyVehicleList', fetchCompanyVehicleList);

  useEffect(() => {
    fetchCompanyVehicleList();
  }, [fetchCompanyVehicleList]);
  // useEffect(() => {
