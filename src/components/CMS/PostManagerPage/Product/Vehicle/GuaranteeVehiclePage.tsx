...
export default function GuaranteeVehiclePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/vehicle/guarantee/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchGuaranteeVehicleList');
      })
      .catch((err) => {
  ...
  const fetchGuaranteeVehicleList = useCallback(async () => {
    await instanceAxios
      .get(`/vehicle/guarantee/`, {
        params: {
  ...
  useSWR('fetchGuaranteeVehicleList', fetchGuaranteeVehicleList);

  useEffect(() => {
    fetchGuaranteeVehicleList();
  }, [fetchGuaranteeVehicleList]);
  // useEffect(() => {
