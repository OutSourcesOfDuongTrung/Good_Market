...
export default function SellerInformationBonsaiFurniturePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/home-appliance/seller-information/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchSellerInformationBonsaiFurnitureList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/home-appliance/seller-information/${id}/`)
      .then((res) => {
  ...
  const fetchSellerInformationBonsaiFurnitureList = useCallback(async () => {
    await instanceAxios
      .get(`/home-appliance/seller-information/`, {
        params: {
  ...
  useSWR(
    'fetchSellerInformationBonsaiFurnitureList',
    fetchSellerInformationBonsaiFurnitureList
  );

  useEffect(() => {
    fetchSellerInformationBonsaiFurnitureList();
  }, [fetchSellerInformationBonsaiFurnitureList, valueFilter]);
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/home-appliance/seller-information/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchSellerInformationBonsaiFurnitureList');
        ...
        columns={columns}
        // refeshWhenNotValue={'fetchSellerInformationBonsaiFurnitureList'}
      />
      <Modal
        styles={{
          mask: {
