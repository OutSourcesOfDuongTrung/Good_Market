...
export default function PostRefrigeratorPage() {
  const [dataList, setDataList] = useState<IRefrigeratorPost[]>([]);
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(
        `/refrigerator-airconditioner-washingmachine/items/${currentID}/`,
        e
      )
      .then((res) => {
        form.resetFields();
        ...
        mutate('fetchPostRefrigeratorList');
      })
      .catch((err) => {
  ...
  const fetchPostRefrigeratorList = useCallback(async () => {
    await instanceAxios
      .get(`/refrigerator-airconditioner-washingmachine/items/`, {
        params: {
  ...
  useSWR('fetchPostRefrigeratorList', fetchPostRefrigeratorList);

  useEffect(() => {
    fetchPostRefrigeratorList();
  }, [fetchPostRefrigeratorList]);
  // useEffect(() => {
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/refrigerator-airconditioner-washingmachine/items/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchPostRefrigeratorList');
