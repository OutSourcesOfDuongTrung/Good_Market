...
export default function ScreenSizeElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/screen_size/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchScreenSizeElectronicDeviceList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/screen_size/${id}/`)
      .then((res) => {
  ...
  const fetchScreenSizeElectronicDeviceList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/screen_size/`, {
        params: {
  ...
  useSWR(
    'fetchScreenSizeElectronicDeviceList',
    fetchScreenSizeElectronicDeviceList
  );
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/ElectronicDevice/screen_size/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchScreenSizeElectronicDeviceList');
