...
export default function ColorElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/color/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchColorElectronicDeviceList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/color/${id}/`)
      .then((res) => {
  ...
  const fetchColorElectronicDeviceList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/color/`, {
        params: {
  ...
  useSWR('fetchColorElectronicDeviceList', fetchColorElectronicDeviceList);

  useEffect(() => {
    fetchColorElectronicDeviceList();
  }, [fetchColorElectronicDeviceList]);
  // useEffect(() => {
