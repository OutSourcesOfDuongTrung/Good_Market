...
export default function HardDriveElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/hard_drive/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchHardDriveElectronicDeviceList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/hard_drive/${id}/`)
      .then((res) => {
  ...
  const fetchHardDriveElectronicDeviceList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/hard_drive/`, {
        params: {
  ...
  useSWR(
    'fetchHardDriveElectronicDeviceList',
    fetchHardDriveElectronicDeviceList
  );
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/ElectronicDevice/hard_drive/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchHardDriveElectronicDeviceList');
