...
export default function CapacitiesElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/capacities/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchCapacitiesElectronicList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/capacities/${id}/`)
      .then((res) => {
  ...
  const fetchCapacitiesElectronicList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/capacities/`, {
        params: {
  ...
  useSWR('fetchCapacitiesElectronicList', fetchCapacitiesElectronicList);

  useEffect(() => {
    fetchCapacitiesElectronicList();
  }, [fetchCapacitiesElectronicList]);
  // useEffect(() => {
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/ElectronicDevice/capacities/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchCapacitiesElectronicList');
