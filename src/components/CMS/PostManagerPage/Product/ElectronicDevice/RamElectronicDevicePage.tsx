...
export default function RamElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/ram/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchRamElectronicDeviceList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/ram/${id}/`)
      .then((res) => {
  ...
  const fetchRamElectronicDeviceList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/ram/`, {
        params: {
  ...
  useSWR('fetchRamElectronicDeviceList', fetchRamElectronicDeviceList);

  useEffect(() => {
    fetchRamElectronicDeviceList();
  }, [fetchRamElectronicDeviceList]);
  // useEffect(() => {
