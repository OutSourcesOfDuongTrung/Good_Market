...
export default function MicroprocessorElectronicDevicePage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/ElectronicDevice/microprocessor/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchMicroprocessorElectronicDeviceList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/ElectronicDevice/microprocessor/${id}/`)
      .then((res) => {
  ...
  const fetchMicroprocessorElectronicDeviceList = useCallback(async () => {
    await instanceAxios
      .get(`/ElectronicDevice/microprocessor/`, {
        params: {
  ...
  useSWR(
    'fetchMicroprocessorElectronicDeviceList',
    fetchMicroprocessorElectronicDeviceList
  );
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/ElectronicDevice/microprocessor/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchMicroprocessorElectronicDeviceList');
