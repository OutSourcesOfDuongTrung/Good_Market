...
export default function CategoryVietShopPage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/shop-viet/category/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchCategoryVietShopList');
      })
      .catch((err) => {
  ...
  const fetchCategoryVietShopList = useCallback(async () => {
    await instanceAxios
      .get(`/shop-viet/category/`, {
        params: {
  ...
  useSWR('fetchCategoryVietShopList', fetchCategoryVietShopList);

  useEffect(() => {
    fetchCategoryVietShopList();
  }, [fetchCategoryVietShopList, valueFilter]);
  ...
  return (
    <div>
      <CMSCategory
        ...
        columns={columns}
        // refeshWhenNotValue={'fetchCategoryVietShopList'}
      />
      <Modal
        styles={{
          mask: {
