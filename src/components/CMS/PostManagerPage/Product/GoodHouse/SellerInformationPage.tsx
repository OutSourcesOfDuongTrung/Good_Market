...
export default function SellerInformationPage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/good-house/seller-information/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchSellerInformationList');
      })
      .catch((err) => {
  ...
  const fetchDelete = async (id: number) => {
    await instanceAxios
      .delete(`/good-house/seller-information/${id}/`)
      .then((res) => {
  ...
  const fetchSellerInformationList = useCallback(async () => {
    await instanceAxios
      .get(`/good-house/seller-information/`, {
        params: {
  ...
  useSWR('fetchSellerInformationList', fetchSellerInformationList);
  ...
  return (
    <div>
      <CMSCategory
        ...
        createAble={true}
        create={{
          url: '/good-house/seller-information/',
          inputName: ['Name'],
          // body: { asdas: 'asdd' },
          onSucces(res) {
            mutate('fetchSellerInformationList');
