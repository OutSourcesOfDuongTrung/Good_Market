...
import { IServices } from '@/types/Job';
import {
  CloseOutlined,
...
export default function PostProductServicePage() {
  const [dataList, setDataList] = useState<IServices[]>([]);
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/service/items/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchPostProductServiceList');
      })
      .catch((err) => {
  ...
  const fetchPostProductServiceList = useCallback(async () => {
    await instanceAxios
      .get(`/service/items/`, {
        params: {
  ...
  useSWR('fetchPostProductServiceList', fetchPostProductServiceList);

  useEffect(() => {
    fetchPostProductServiceList();
  }, [fetchPostProductServiceList]);
  // useEffect(() => {
