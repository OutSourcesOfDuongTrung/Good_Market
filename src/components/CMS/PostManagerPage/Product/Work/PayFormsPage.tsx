...
export default function PayFormsPage() {
  const [PayFormList, setPayFormList] = useState<IJob[]>([]);
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/job/pay-forms/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchPayFormList');
      })
      .catch((err) => {
  ...
  const fetchPayFormList = useCallback(async () => {
    await instanceAxios
      .get(`/job/pay-forms/`, {
        params: {
      ...
      .then((res) => {
        ...
        setPayFormList(res.data.data || []);
      })
      .catch((err) => {
        console.log(err);
      });
  }, [currentPage, valueFilter]);
  useSWR('fetchPayFormList', fetchPayFormList);
