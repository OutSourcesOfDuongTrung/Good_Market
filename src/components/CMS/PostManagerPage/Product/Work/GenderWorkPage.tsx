...
export default function GenderWorkPage() {
  ...
  const onFinish = async (e: any) => {
    await instanceAxios
      .patch(`/job/sex/${currentID}/`, e)
      .then((res) => {
        form.resetFields();
        setOpenModalCreate(false);
        message.success('Đã cập nhật');
        mutate('fetchGenderWorkList');
      })
      .catch((err) => {
  ...
  const fetchGenderWorkList = useCallback(async () => {
    await instanceAxios
      .get(`/job/sex/`, {
        params: {
  ...
  useSWR('fetchGenderWorkList', fetchGenderWorkList);
